...
def login(b):  #登录京东
    ...
    time.sleep(1)
    b.fill("loginname","XXXXXXXXX")  #填写账户密码
    b.fill("nloginpwd","XXXXXXXXX")
...
def buy_time(buytime):
    while True:
        ...
        if now.strftime('%Y-%m-%d %H:%M:%S') == buytime:
            while True:
                b.find_by_id("choose-btns").click()  # 找到抢购按钮，点击
...
b.visit("https://item.jd.com/100015627006.html")
login(b)
#获取现在时间
now = datetime.datetime.now()
print(now.strftime('%Y-%m-%d %H:%M:%S'))
#设置抢购的时间
buy_time('2020-11-26 10:00:00')